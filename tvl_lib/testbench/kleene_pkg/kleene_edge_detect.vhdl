...
entity kleene_edge_detect_tb is
  generic (runner_cfg : string);
end entity;

architecture test of kleene_edge_detect_tb is
  
  signal test_sig  : KLEENE := false;
...
begin
  ...
  main : process
    ...
    variable from_value : KLEENE;
    variable to_value   : KLEENE;
  ...
  begin
    ...
    if run("Test transitions") then
      test_transition(false, unk, expect_mz_rising  => true,
                                expect_any_rising => true);
      test_transition(unk, true, expect_zp_rising  => true,
                                expect_any_rising => true);
      test_transition(false, true, expect_mp_rising  => true,
                                expect_any_rising => true);

      test_transition(true, unk, expect_pz_falling  => true,
                                expect_any_falling => true);
      test_transition(unk, false, expect_zm_falling  => true,
                                expect_any_falling => true);
      test_transition(true, false, expect_pm_falling  => true,
                                expect_any_falling => true);

    elsif run("Test no event detection") then

      -- Testing that stable signals don't trigger edge detection

      test_transition(false, false);
      test_transition(unk, unk);
      test_transition(true, true);
      
    elsif run("Test mutual exclusivity") then

      -- Testing that rising and falling edges are mutually exclusive
      
      -- Test all 9 possible transitions
      for from_idx in 0 to 2 loop
        for to_idx in 0 to 2 loop
          case from_idx is
            when 0 => from_value := false;
            when 1 => from_value := unk;
            when 2 => from_value := true;
            when others => null;
          end case;
          
          case to_idx is
            when 0 => to_value := false;
            when 1 => to_value := unk;
            when 2 => to_value := true;
            when others => null;
          end case;
          ...
          check_false(any_rising_detected and any_falling_detected,
                      ...
                      KLEENE'image(from_value) & " -> " &
                      KLEENE'image(to_value));
        end loop;
      end loop;
